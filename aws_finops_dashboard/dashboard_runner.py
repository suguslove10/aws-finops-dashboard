...
def _run_ri_optimizer(profiles_to_use: List[str], args: argparse.Namespace) -> None:
    """Generate Reserved Instance and Savings Plan recommendations."""
    console.print("[bold bright_cyan]Generating Reserved Instance and Savings Plan recommendations...[/]")
    ...
    for profile in profiles_to_use:
        ...
        # Run RI optimizer
        optimizer = RIOptimizer(session, args.lookback_days or 30)
        ...
        if ri_recommendations['recommendations']:
            ...
            ri_table.add_column("Service", style="bold magenta")
            ri_table.add_column("Instance Type", style="cyan")
            ri_table.add_column("Region", style="blue")
            ri_table.add_column("Count", justify="right", style="green")
            ri_table.add_column("Term", style="yellow")
            ri_table.add_column("Monthly Savings ($)", justify="right", style="green")
            ...
            console.print(ri_table)
            console.print(f"Total estimated monthly savings from RIs: [bold green]${ri_recommendations['estimated_savings']:.2f}[/]")
        else:
            console.print("[yellow]No Reserved Instance recommendations available.[/]")
        
        # Display Savings Plan recommendations
        if sp_recommendations['recommendations']:
            sp_table = Table(
                title=f"Savings Plan Recommendations for {profile} (Account: {account_id})",
                box=box.ASCII_DOUBLE_HEAD,
                style="bright_cyan",
                show_lines=True
            )
            
            sp_table.add_column("Plan Type", style="bold magenta")
            sp_table.add_column("Hourly Commitment", style="cyan")
            sp_table.add_column("Term", style="yellow")
            sp_table.add_column("Monthly Savings ($)", justify="right", style="green")
            sp_table.add_column("Utilization", style="blue")
            ...
            console.print(sp_table)
            console.print(f"Total estimated monthly savings from Savings Plans: [bold green]${sp_recommendations.get('estimated_total_savings', 0):.2f}[/]")
        ...
        console.print(f"\n[bold green]Total potential monthly savings: ${total_monthly_savings:.2f}[/]")
        console.print(f"[bold green]Total potential annual savings: ${total_monthly_savings * 12:.2f}[/]")
        
        # Export recommendations if requested
        if args.report_name and args.report_type:
            console.print(f"[cyan]Preparing to export recommendations for profile {profile}[/]")
            console.print(f"[cyan]Report name: {args.report_name}[/]")
            console.print(f"[cyan]Report types: {args.report_type}[/]")
            
            for report_type in args.report_type:
                console.print(f"[cyan]Exporting as {report_type}...[/]")
                
                if report_type == "json":
                    _export_ri_recommendations_to_json(
                        profile, 
                        account_id,
                        ri_recommendations, 
                        sp_recommendations, 
                    ...
                    _export_ri_recommendations_to_csv(
                        profile,
                        account_id, 
                        ri_recommendations, 
                        sp_recommendations, 
                    ...
                    console.print(f"[cyan]Calling PDF export function...[/]")
                    try:
                        ...
                        console.print(f"[cyan]PDF export function completed[/]")
                    except Exception as e:
                        console.print(f"[bold red]Error in PDF export: {str(e)}[/]")
                        import traceback
                        console.print(traceback.format_exc())


def _export_ri_recommendations_to_json(
    profile: str,
    account_id: str,
    ri_recommendations: Dict[str, Any],
    sp_recommendations: Dict[str, Any],
    report_name: str,
    output_dir: Optional[str] = None
) -> None:
    """Export RI and SP recommendations to JSON format."""
    import json
    import os
    from datetime import datetime
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    filename = f"{report_name}_{profile}_ri_recommendations_{timestamp}.json"
    ...
    data = {
        ...
        "total_monthly_savings": ri_recommendations['estimated_savings'] + sp_recommendations.get('estimated_total_savings', 0),
        "total_annual_savings": (ri_recommendations['estimated_savings'] + sp_recommendations.get('estimated_total_savings', 0)) * 12
    }
    
    try:
        with open(filepath, "w") as f:
            json.dump(data, f, indent=2)
        console.print(f"[bright_green]RI recommendations exported to JSON: {filepath}[/]")
    except Exception as e:
        console.print(f"[bold red]Error exporting RI recommendations to JSON: {str(e)}[/]")


def _export_ri_recommendations_to_csv(
    profile: str,
    account_id: str,
    ri_recommendations: Dict[str, Any],
    sp_recommendations: Dict[str, Any],
    ...
    """Export RI and SP recommendations to CSV format."""
    import csv
    import os
    from datetime import datetime
    ...
    # Export RI recommendations
    if ri_recommendations['recommendations']:
        ri_filename = f"{report_name}_{profile}_ri_recommendations_{timestamp}.csv"
        ri_filepath = os.path.join(output_dir, ri_filename) if output_dir else ri_filename
        
        try:
            with open(ri_filepath, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "Profile", "Account ID", "Service", "Instance Type", "Region", 
                    "Recommended Count", "Term", "Payment Option", "Monthly Savings ($)",
                    "Confidence", "Utilization Projection"
                ])
                ...
                for rec in ri_recommendations['recommendations']:
                    writer.writerow([
                        ...
                        f"{rec['monthly_savings']:.2f}",
                        rec['confidence'],
                        rec['utilization_projection']
                    ])
            console.print(f"[bright_green]RI recommendations exported to CSV: {ri_filepath}[/]")
        except Exception as e:
            console.print(f"[bold red]Error exporting RI recommendations to CSV: {str(e)}[/]")
    
    # Export SP recommendations
    if sp_recommendations['recommendations']:
        sp_filename = f"{report_name}_{profile}_sp_recommendations_{timestamp}.csv"
        sp_filepath = os.path.join(output_dir, sp_filename) if output_dir else sp_filename
        
        try:
            with open(sp_filepath, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([
                    "Profile", "Account ID", "Plan Type", "Hourly Commitment ($)", 
                    "Monthly Commitment ($)", "Term", "Payment Option", 
                    "Monthly Savings ($)", "Estimated Utilization", "Confidence"
                ])
            ...
            console.print(f"[bright_green]SP recommendations exported to CSV: {sp_filepath}[/]")
        except Exception as e:
            console.print(f"[bold red]Error exporting SP recommendations to CSV: {str(e)}[/]")


def _export_ri_recommendations_to_pdf(
    profile: str,
    account_id: str,
    ri_recommendations: Dict[str, Any],
    sp_recommendations: Dict[str, Any],
    ...
    """Export RI and SP recommendations to PDF format."""
    import os
    from datetime import datetime
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    filename = f"{report_name}_{profile}_ri_recommendations_{timestamp}.pdf"
    ...
    try:
        ...
        # Add title
        elements.append(Paragraph(f"AWS Reserved Instance & Savings Plan Recommendations", styles["Title"]))
        elements.append(Spacer(1, 12))
        
        # Add profile info
        elements.append(Paragraph(f"Profile: {profile}", styles["Heading2"]))
        elements.append(Paragraph(f"Account ID: {account_id}", styles["Normal"]))
        ...
        elements.append(Paragraph(f"Total potential monthly savings: ${total_monthly_savings:.2f}", styles["Normal"]))
        elements.append(Paragraph(f"Total potential annual savings: ${total_monthly_savings * 12:.2f}", styles["Normal"]))
        elements.append(Spacer(1, 12))
        
        # Add RI recommendations
        if ri_recommendations['recommendations']:
            elements.append(Paragraph("Reserved Instance Recommendations", styles["Heading2"]))
            elements.append(Spacer(1, 6))
            
            # Create table
            ri_data = [["Service", "Instance Type", "Region", "Count", "Term", "Monthly Savings ($)", "Confidence"]]
            for rec in ri_recommendations['recommendations']:
                ri_data.append([
                    rec['service'],
                    rec['instance_type'],
            ...
            elements.append(Spacer(1, 12))
            elements.append(Paragraph(f"Total estimated monthly savings from RIs: ${ri_recommendations['estimated_savings']:.2f}", styles["Normal"]))
            elements.append(Spacer(1, 12))
        
        # Add SP recommendations
        if sp_recommendations['recommendations']:
            elements.append(Paragraph("Savings Plan Recommendations", styles["Heading2"]))
            elements.append(Spacer(1, 6))
            
            # Create table
            sp_data = [["Plan Type", "Hourly Commitment", "Term", "Payment Option", "Monthly Savings ($)", "Utilization"]]
            for rec in sp_recommendations['recommendations']:
                sp_data.append([
                    rec['plan_type'],
                    f"${rec['hourly_commitment']:.2f}/hr",
            ...
            elements.append(Spacer(1, 12))
            elements.append(Paragraph(f"Total estimated monthly savings from Savings Plans: ${sp_recommendations.get('estimated_total_savings', 0):.2f}", styles["Normal"]))
            elements.append(Spacer(1, 12))
        
        # Add guidance
        elements.append(Paragraph("Guidance", styles["Heading2"]))
        elements.append(Paragraph("These recommendations are based on historical usage patterns. Before purchasing Reserved Instances or Savings Plans:", styles["Normal"]))
        elements.append(Spacer(1, 6))
        
        guidance_data = [
            ["1.", "Verify that the recommended instances match your workload requirements."],
            ["2.", "Consider your future capacity needs and growth plans."],
            ["3.", "For Reserved Instances, decide between Standard or Convertible RIs based on your flexibility needs."],
            ["4.", "For Savings Plans, Compute Savings Plans offer more flexibility than EC2 Instance Savings Plans."],
            ["5.", "Consider a mix of 1-year and 3-year commitments to balance savings and flexibility."]
        ]
        
        guidance_table = Table(guidance_data, colWidths=[30, 450])
        ...
        # Make sure we have at least some content to generate a PDF
        if not elements:
            elements.append(Paragraph("No recommendations available", styles["Title"]))
        
        # Build the document
        doc.build(elements)
        console.print(f"[bright_green]Recommendations exported to PDF: {filepath}[/]")
    except Exception as e:
        console.print(f"[bold red]Error exporting recommendations to PDF: {str(e)}[/]")
        import traceback
        console.print("[bold red]Error details:[/]")
        console.print(traceback.format_exc())
        
        # Additional debugging information
        console.print(f"[bold yellow]Debug info:[/]")
        console.print(f"Profile: {profile}")
        console.print(f"Account ID: {account_id}")
        console.print(f"Output directory: {output_dir}")
        console.print(f"Target file path: {filepath}")
        console.print(f"RI recommendations count: {len(ri_recommendations.get('recommendations', []))}")
        console.print(f"SP recommendations count: {len(sp_recommendations.get('recommendations', []))}")
